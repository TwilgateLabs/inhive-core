...
var baseline = map[string]int{
	// 2026-07-26: 10 → 8. Ушли два необъяснённых гейта вокруг памяти: минутный
	// принудительный GC по таймеру на обеих мобильных платформах выкинут
	...
	// routing loop to exact TUN address on darwin» (смысл: на darwin точное
	// совпадение с адресом TUN тоже считать своим, иначе пакет уходит в петлю;
	// на остальных ОС такое совпадение — легальный пункт назначения).
	// Почему подняли, а не прокомментировали: комментарий внутри апстримной
	// функции стал бы точкой конфликта на каждом следующем merge, а править
	// чужой гейт ради нашего гейта — плохая сделка. Правило «комментируй гейт»
	// адресовано НАШЕМУ коду; для унаследованного апстрима и существует baseline.
	"sing-box":  35, // унаследованный апстрим + наши патчи поверх
	"cmd":       0,
	"platform":  0,
	"xray2sing": 0,
}
